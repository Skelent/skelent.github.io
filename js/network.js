...
function Server( socketio, slots )
{
    ...
    app.post("/compress", function (request, response) {
      try {
        const buffer = Buffer.from(request.query.data || request.body.data, 'base64');
        zlib.gzip(buffer, function(err, buf) {
          if (err) {
            ...
            response.send(buf.toString('base64'));
            response.end();
          }
        });
      } catch (err) {
...
Server.prototype.onBlockUpdate = function( socket, data )
{
	...
	if ( Math.sqrt( (data.x-world.spawnPoint.x)*(data.x-world.spawnPoint.x) + (data.y-world.spawnPoint.y)*(data.y-world.spawnPoint.y) + (data.z-world.spawnPoint.z)*(data.z-world.spawnPoint.z)  ) < (world.sx + world.sy) / 16 ) return false;
